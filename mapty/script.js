...
class Workout {
  date = new Date();
  id = Math.trunc(Math.random() * (Date.now() + " ").slice(-10));
  clocks = 0;
...
class App {
  ...
  #mapZoomlLevel = 13;
  constructor() {
    //to autoload function declare it inside constructor
    ///get position
    this._getPosition();
    //load local storage
  ...
  _loadMap(position) {
    ...
    this.#map = L.map("map").setView(cords, this.#mapZoomlLevel);
    ...
    L.marker(cords).addTo(this.#map).bindPopup().openPopup();
  ...
  _newWorkout(e) {
    ...
    const acllpositive = (...input) => input.every((inp) => inp > 0);
    e.preventDefault();
    //get data
    ...
    if (type === "running") {
      ...
      if (
        !validInput(distane, duration, cadence) ||
        !acllpositive(distane, duration, cadence)
      )
        alert("Input have to be posiyive number");
      workout = new Running([lat, lng], distane, duration, cadence);
    }
    //if workout is cycling create cycling object
    if (type === "cycling") {
      const elevartion = +inputElevation.value;
      if (
        !validInput(distane, duration, elevartion) ||
        !acllpositive(distane, duration)
      )
        alert("Input have to be posiyive number");
      workout = new Cycling([lat, lng], distane, duration, elevartion);
    }
  ...
  _moveToPopup(e) {
    ...
    this.#map.setView(workout.coords, this.#mapZoomlLevel, {
      animate: true,
      pan: { duration: 1 },
    });
    workout.click();
